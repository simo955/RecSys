...
class ItemKNNRatingHybridRecommender4(SimilarityMatrixRecommender, Recommender):
    """ ItemKNNRatingHybridRecommender4
    Hybrid of two similarities S = S1*alpha + S2*(1-alpha)

    """

    RECOMMENDER_NAME = "ItemKNNRatingHybridRecommender4"


    def __init__(self, URM_train, Recommender_1, Recommender_2, Recommender_3,Recommender_4,userID, norm = "classic",sparse_weights=True):
        super(ItemKNNRatingHybridRecommender4, self).__init__()
        ...
        ratingsMatrix3=Recommender_3.recommendBatch(userID)
        
        #Get Similarity matrix (W_sparse) from Recommender4 and normalize its value for its max
        ratingsMatrix4=Recommender_4.recommendBatch(userID)
        
        if norm == "classic":
            ratingsMatrix1=np.asmatrix(ratingsMatrix1/ratingsMatrix1.max())
            ratingsMatrix2=np.asmatrix(ratingsMatrix2/ratingsMatrix2.max())
            ratingsMatrix3=np.asmatrix(ratingsMatrix3/ratingsMatrix3.max())
            ratingsMatrix4=np.asmatrix(ratingsMatrix4/ratingsMatrix4.max())
        ...
        if norm == "XX":
            ...
            vectorMax3=ratingsMatrix3.max(axis=1)
            ratingsMatrix3=np.asmatrix(ratingsMatrix3/vectorMax3[:,None])
            
            vectorMax4=ratingsMatrix4.max(axis=1)
            ratingsMatrix4=np.asmatrix(ratingsMatrix4/vectorMax4[:,None])
    ...
    def fit(self, alpha = 0.5,beta =0.2,gamma =0.2,delta=0.1):
        self.ratingsMatrix = (self.RatingsMatrix1*alpha + self.RatingsMatrix2*(beta)+ self.RatingsMatrix3*(gamma)+ self.RatingsMatrix4*(delta)).toarray()

...
class ItemKNNRatingHybridRecommender3(SimilarityMatrixRecommender, Recommender):
    """ ItemKNNRatingHybridRecommender3
    Hybrid of two similarities S = S1*alpha + S2*(1-alpha)

    """

    RECOMMENDER_NAME = "ItemKNNRatingHybridRecommender3"


    def __init__(self, URM_train, Recommender_1, Recommender_2, Recommender_3, userID, norm="classic", sparse_weights=True):
        super(ItemKNNRatingHybridRecommender3, self).__init__()
    ...
    def fit(self, alpha = 0.5,beta =0.2, gamma= 0.2):
        self.ratingsMatrix = (self.RatingsMatrix1*alpha + self.RatingsMatrix2*(beta)+ self.RatingsMatrix3*(gamma)).toarray()
